...
- 表空间由`页`组成：
  - 每64 个页被划分成一个`区`(16K * 64 = 1MB)。
  - 每256个区被划分成一个`组`(1MB * 256 = 256MB)。
  
- 每个组第一个区的第一个页中有256个`XDES Entry`结构，用来管理组内对应的区，包括：
  - 区的状态：
    - FREE：空闲的区。
    - FREE_FRAG：有剩余空间的碎片区。
    - FULL_FRAG：没有剩余空间的碎片区。
    - FSEG：附属于某个段的区。
  - 区内64个页面的空闲状态。
  - 能让区形成双向链表的前一个和后一个区的指针。

- 第一个组第一个区的第一个页中除了有256个`XDES Entry`结构，还有表空间的一些整体信息：
  - FREE链表的基节点(`XDES Entry`链表)(直属表空间)。
  - FREE_FRAG链表的基节点(`XDES Entry`链表)(直属表空间)。
  - FULL_FRAG链表的基节点(`XDES Entry`链表)(直属表空间)。
  - SEG_INODES_FULL链表的基节点(存放段信息页的链表)。
  - SEG_INODES_FREE链表的基节点(存放段信息页的链表)。

- 第一个组第一个区的第三个页是段信息页，包括：
  - 能让段信息页形成双向链表的上一个页面和下一个页面的指针。
  - 还有84个`INODE Entry`结构，记录每个段的信息：
    - 段的FREE链表基节点(`XDES Entry`链表)。
    - 段的NOT_FULL链表基节点(`XDES Entry`链表)。
    - 段的FULL链表基节点(`XDES Entry`链表)。
    - 段对应32个零散页面的页号。

- 系统表空间中第7个页(数据字典)保存了各个元数据表的根页面的页号，通过元数据表可以找到某个表的表空间文件，以及表的聚簇索引和二级索引的根页面页号。
- 在B+树索引的根节点页的页头中保存了叶子节点段和非叶子节点段对应的`INODE Entry`结构的页号和偏移量。
